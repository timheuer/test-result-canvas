...
The extension registers a **Test Results** canvas (`test-result-viewer`) that scans the current workspace for `.trx` files, parses TRX test run output, and shows a focused test ledger with summary counts, filtering, search, durations, failure messages, stack traces, captured output, and run metadata.
